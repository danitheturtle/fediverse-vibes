...
export const useSharkey = (apiToken) => {
  ...
  const [followerIds, _setFollowerIds] = useState([]);

  const setFollowerIds = (newFollowerIds) => {
    _setFollowerIds(newFollowerIds);
    localStorage.setItem("followerIdList", JSON.stringify(newFollowerIds));
  }
  useEffect(() => {
