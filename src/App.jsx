...
const bypassSpamUserIds = ["9m6ts5o3t85y0034"];
        ...
        .forEach((notif) => {
          ...
          oldNotifications.slice(-10).forEach((oldNotif) => {
            if (oldNotif?.user?.id === notif?.user?.id) {
              if (!bypassSpamUserIds.includes(notif?.user?.id)) {
                spamFlag++;
              }
            }
          });
          ...
          if (notif.type === "follow") {
            if (!bypassSpamUserIds.includes(notif?.user?.id)) {
              if (notif?.user?.id?.length > 0 && followerIds.includes(notif?.user?.id)) {
                ...
                if (followerIds) {
                  setFollowerIds([...followerIds, notif?.user?.id]);
                }
              }
            }
          }
          ...
          if (
            ...
            updateSettings = settings[updateSettings.fallbackType]
              ...
              : settings.default;
          } else if (updateSettings.fallbackType) {
            timerScale = Math.min(Math.max((notif?.note?.text?.length*2) / updateSettings.minCharsToCount, 1.1), 8);
          }
          console.dir(timerScale);

          if (updateSettings.vibeIndex === "all") {
            motorState.forEach((motor) => {
              // console.dir(motor.intensity);
              motor.timer = motor.timer + updateSettings.secondsPerNotification*timerScale;
              motor.intensity =
            ...
            thisMotor.timer =
              thisMotor.timer + updateSettings.secondsPerNotification*timerScale;
            thisMotor.intensity =
